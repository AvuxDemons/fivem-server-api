...
async function main() {
	...
	const name = (info.vars?.sv_projectName || "").replace(/^\^\d+/, "");
	const desc = info.vars?.sv_projectDesc || "";
	const locale = info.vars?.locale || "";
	const gamename = info.vars?.gamename || "";
	const build = Number(info.vars?.sv_enforceGameBuild || 0);
	const max = Number(info.vars?.sv_maxClients || 0);
	const tags = info.vars?.tags || "";
	const onesync = info.vars?.onesync_enabled === "true";
